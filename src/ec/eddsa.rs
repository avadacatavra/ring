...
mod tests {
    ...
    fn test_signature_ed25519() {
        ...
        /*
        Go out of the current page since it has the return address to the caller. We don't want to ruin that!
        */
        create_padding!();
        ...
        test::from_file("src/ec/ed25519_tests.txt", |section, test_case| {
            ...
            /*
            Define the page that needs to become read-only. Some of the data might also be isolated in a non-accessible page but it depends on how the callee works. I'm not sure at the moment.
            */
            create_padding!();
            ...
            immutable_single_stack_page(&private_key);
            disable_mprotect_stack(&kernel_key, &memory_page_addr_usize!(private_key));
            /*
            Create a new page for the rest of the data below. Possible to put everything on a single read-only page but I don't want to mess with the code.
            */
            ...
            immutable_single_stack_page(&expected_sig);
            disable_mprotect_stack(&kernel_key, &memory_page_addr_usize!(private_key));
            ...
            enable_mprotect_stack(&kernel_key);
            mutable_single_stack_page(&private_key);
            mutable_single_stack_page(&expected_sig);
